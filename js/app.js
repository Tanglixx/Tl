...
// 底部Tab页面切换逻辑
const tabItems = document.querySelectorAll('.main-tab-item');
const pageMap = {
    chat: document.querySelector('.main-chat-area'),
    friend: document.querySelector('.friend-page'),
    discover: document.querySelector('.discover-page'),
    setting: document.querySelector('.setting-page')
};
tabItems.forEach(tab => {
    tab.addEventListener('click', () => {
        // 清空所有激活状态
        tabItems.forEach(t => t.classList.remove('active'));
        Object.values(pageMap).forEach(page => page.classList.remove('active'));
        // 激活当前标签+对应页面
        tab.classList.add('active');
        const targetTab = tab.dataset.tab;
        pageMap[targetTab].classList.add('active');
    })
})

// 发现页入口打开对应弹窗
document.getElementById('openMoments')?.addEventListener('click', ()=>{
    document.querySelector('.moments-container').classList.add('active')
})
document.getElementById('openPet')?.addEventListener('click', ()=>{
    document.querySelector('.pet-container').classList.add('active')
})
document.getElementById('openShop')?.addEventListener('click', ()=>{
    document.querySelector('.shop-container').classList.add('active')
})
// 联系人访客入口
document.getElementById('openVisitorPanel')?.addEventListener('click', ()=>{
    document.querySelector('.visitor-panel').classList.add('active')
})
// 设置美化入口
document.getElementById('openBeautify')?.addEventListener('click', ()=>{
    document.querySelector('.beautify-panel').classList.add('active')
})