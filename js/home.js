...
// 返回会话列表（聊天页返回按钮调用）
window.backChatToList = function(){
    document.getElementById('chat-list-page').style.display = 'block';
    // 清空聊天容器、隐藏聊天主区域
    const chatWrap = document.querySelector('.main-chat-area');
...
function renderChatList(){
    ...
    if(!container) return;
    // 过滤掉自己的会话，只展示角色会话
    const validSessions = sessionList.filter(item => item.id !== 'me');
    if(validSessions.length === 0){
        container.innerHTML = `<div style="text-align:center;padding:40px;color:var(--text-secondary);">暂无会话</div>`;
        return;
    }
    let html = '';
    validSessions.forEach(session => {
        ...
        const status = session.status || "在线";
        html += `
        <div class="chat-list-item" data-sid="${session.id}" onclick="switchSessionByList('${session.id}')">
            <div class="list-avatar">
                ${avatar ? `<img src="${avatar}">` : `<i class="fas fa-user" style="color:var(--accent-color);font-size:20px;"></i>`}
            </div>
...
// 点击列表切换会话
window.switchSessionByList = function(sid){
    location.hash = sid;
}
// 打开会话列表（替换原来直接进聊天的逻辑）
window.openChatListPage = function(){
    hideHomePage();
    document.getElementById('chat-list-page').style.display = 'block';
    renderChatList();
}