        ...
        importAllBtn.addEventListener('click', function() {
            ...
            input.onchange = async function(e) {
                ...
                if (file.size > 220 * 1024 * 1024) {
                    if (typeof showNotification === 'function') showNotification('文件过大，请检查备份文件', 'error');
                    return;
                }
                ...
                try {
                    ...
                    var okShape = backup.type === 'chatapp-backup-v5' ||
                        ...
                        backup.localforage ||
                        backup.indexedDB;
                    if (!okShape) throw new Error('非本程序备份文件');

                    if (!confirm('导入将覆盖对应数据。确定继续吗？')) return;
