...
window.viewEnvLetter = function(section, id) {
    ...
    if (section === 'outbox') {
        const partnerName = (typeof settings !== 'undefined' && settings.partnerName) ? settings.partnerName : '亲爱的';
...
window.replyToEnvLetter = function() {
    ...
    if (replyBtn.dataset.mode === 'sending') {
        ...
        const replyLetter = {
            ...
            replyTime: replyTime,
            status: 'pending',
            replyToId: editingEnvId,
            replyToSection: editingEnvSection
        };
...
function handleSendEnvelope() {
    ...
    if (scheduleSend && scheduleTime) {
        ...
        letterObj.sendToChat = sendToChat;
    }
